<template>
  <div>
    <s-card>
      <div class="ico-list">
        <div v-for="(item, index) in list" :key="index" class="ico-wrapper" :title="selectList.includes(item) ? '这里单独设置了颜色样式！' : '' ">
          <s-icon :name="item" class="ico"  :color="selectList.includes(item) ?'#B02324':''"></s-icon>
